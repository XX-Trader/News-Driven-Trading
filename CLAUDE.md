...
- **日期**：2025-11-20 北京时间
- **更新内容**：
  - ✨ 修复推文数据解析问题（正确提取 text 和 author）
  - ✅ 优化 `_ai_worker`：健壮地提取推文内容，处理字典类型的 author
  - ✅ 增强去重机制：基于 `processed_ids` (历史) 和 `tweet_status` (内存) 双重去重
  - ✅ 完善重试逻辑：实现最大 3 次重试，处理超时和错误状态
  - ✅ 新增僵尸任务清理：自动检测并重置卡死超过 5 分钟的任务
  - ✅ 优化过期清理：自动清理超过 30 分钟的内存状态
...
| **v1.4.0** | AI 分析流程优化与健壮性提升（数据解析修复 + 状态机增强） | ✅ 已完成 |
| **v1.5.0** | 动态触发框架（K线信号动态触发 + 10分钟窗口 + 5秒间隔） | 中 |
| **v2.0.0** | 置信度过滤 + 持仓聚合 + 风险限额 + 多策略 | 低 |

### ⚠️ 缺失功能与规划 (详见 MISSING_FEATURES.md)
- **v1.1.0**: 真实的 Twitter API 接入 (目前为本地 JSON 降级)
- **v1.5.0**: 动态触发框架 (目前为固定 10 秒轮询)
- **v2.0.0**: 置信度过滤 (目前所有信号均下单)
- **v2.0.0+**: 高级风控与订单管理 (目前仅市价单 + 基础止损止盈)
- **v1.x**: 动态用户映射表 (目前硬编码 10 人)
